...
(() => {
  ...
  function buildPeople() {
    ...
    byId.forEach((p) => {
      ...
      const average = p.sum / p.count;                       // สูตรที่ใช้
  ...
  function configPanel() {
    ...
    return `<div class="card wide" style="margin-bottom:18px;background:linear-gradient(135deg,#f8fafc,#eff6ff);border:1px solid #dbeafe;padding:16px 20px;">
      ...
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(165px,1fr));gap:12px;">
        ${numberField('v3IncMinHours', 'ชั่วโมงเฉลี่ยขั้นต่ำ/วัน', cfg.minHours, '0.5', '0', '24', 'ชั่วโมงทำงานเฉลี่ยต่อวันที่เข้าเฉลี่ย', false)}
  ...
  function tierCards() {
    ...
    ${fixed ? '' : `<div class="sub" style="margin:-8px 0 16px;">โหมดจ่ายตามหน่วยหยิบเกินเป้าใช้อัตราเดียวกันทุก Tier (เหมือน V2) ระดับ Tier จึงเป็นแค่ป้ายบอกระดับ
      · เงินรางวัล = ผลรวม<b>รายแถวที่เข้าเฉลี่ย</b> ของ max(0, Total Pick ของแถวนั้น − ชั่วโมงทำงานของแถวนั้น × Target โซน) × ${esc(fmt(cfg.unitRate, 2))} บาท
      · แถวที่ไม่เข้าเฉลี่ยจะไม่เข้าสูตรนี้ ทั้งที่คอลัมน์ “Total Pick” ในตารางรวมแถวเหล่านั้นไว้ด้วย
  ...
  function remarkReasons(x) {
    ...
    if (x.zone.key === 'unknown') parts.push('ค่าโซนในข้อมูลไม่ตรงกฎโซนของ V1 จึงเทียบ Target รวม');
    return parts;
  }
  ...
  function renderPage() {
    ...
    if (!list.length) {
      host.innerHTML = configPanel() + dayLimitNotice()
        + '<div class="card v3-card"><div class="staff-miss-ok">ยังไม่มีพนักงานที่นับ Productivity ได้ในช่วงที่เลือก (ต้องมีแถวที่นับได้อย่างน้อยหนึ่งแถว)</div></div>';
      bindConfig();
      return;
    }
    ...
    const hoursFactText = 'เกณฑ์ชั่วโมงมองเฉพาะแถวที่เข้าเฉลี่ย '
      + (hoursFacts.shortCounted
        ? `ช่วงนี้มีแถวเข้าเฉลี่ยที่ชั่วโมงทำงานต่ำกว่า ${fmt(SHORT_HOURS)} ชม. ${fmt(hoursFacts.shortCounted)} แถว`
        : `ช่วงนี้ไม่มีแถวเข้าเฉลี่ยไหนที่ชั่วโมงทำงานต่ำกว่า ${fmt(SHORT_HOURS)} ชม.`)
      + (hoursFacts.minCountedHours !== null ? ` (ต่ำสุด ${fmt(hoursFacts.minCountedHours, 1)} ชม.)` : '')
      + (hoursFacts.shortNotCount
        ? ` ส่วนแถวชั่วโมงสั้นที่ต้นทางไม่นับเข้าเฉลี่ยมี ${fmt(hoursFacts.shortNotCount)} แถว ซึ่งไม่เข้าค่าเฉลี่ยและไม่เข้าเกณฑ์ชั่วโมงของหน้านี้`
        : '')
    ...
    host.innerHTML = configPanel()
      ...
      + `<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(420px,1fr));gap:18px;margin:18px 0;">
        ...
        <div class="card wide" style="margin:0;">
          ...
          <div class="sub">สังกัดยึดทะเบียนพนักงาน ถ้าไม่มีจึงใช้สังกัดในข้อมูลผลงาน ที่พบบ่อยที่สุดของคนนั้น</div>
      ...
      <div class="note v3-notice">
        ...
        <b>ที่มาของตัวเลข</b> · Productivity รายคน = ผลรวมค่าเฉลี่ยต่อชั่วโมงของแถวที่นับได้ ÷ จำนวนแถวนั้น (รวมครั้งเดียว ไม่เอาค่าเฉลี่ยมาเฉลี่ยซ้ำ)
        · % ของเป้า = Productivity ÷ Target ของโซนหลัก × 100 โดยโซนหลักคือโซนที่มีแถวเข้าเฉลี่ยมากที่สุด
        · <b>เท่ากับเป้านับว่าผ่าน</b> · Total Pick = ผลรวมยอดหยิบของทุกแถวที่มีวันที่ รวมแถวที่ไม่เข้าเฉลี่ย
        · คนที่ไม่มีแถวเข้าเฉลี่ยเลย (นับได้ 0 แถว) ไม่ถูกตัดสินผ่าน/ไม่ผ่าน จึงไม่อยู่ในหน้านี้
        ...
        <b>สูตรเงินรางวัลของโหมดที่เลือกอยู่</b> · ${fixed
          ...
          : `ผลรวม<b>รายแถวที่เข้าเฉลี่ย</b> ของ max(0, Total Pick ของแถวนั้น − ชั่วโมงทำงานของแถวนั้น × Target โซน) แล้วคูณ ${esc(fmt(cfg.unitRate, 2))} บาท/หน่วย
            · คิดรายแถวเพื่อไม่ให้แถวที่ต่ำกว่าเป้ามาหักล้างแถวที่เกินเป้า (V2 คิดรวมทั้งช่วงจึงหักล้างกันได้)
            · <b>แถวที่ไม่เข้าเฉลี่ยจะไม่เข้าสูตรนี้</b> ทั้งที่คอลัมน์ “Total Pick” ในตารางรวมแถวเหล่านั้นไว้
        ...
        · ชั่วโมงเฉลี่ย/วัน = ชั่วโมงทำงานของแถวที่เข้าเฉลี่ย ÷ จำนวนวันที่เข้าเฉลี่ย (หนึ่งวันมีได้หลายแถว จึงไม่หารด้วยจำนวนแถว)
        ...
        <b>คนที่ออกแล้ว</b> · ${esc(fmt(resignedAll))} คนในตารางอยู่ในรายชื่อที่ลาออก หรือถูกกรอกในเว็บ ขึ้น Remark แดงไว้
        ${cfg.hideResigned
        ...
        ${unknownZone ? `<br><b>โซน Not Found</b> · ${esc(fmt(unknownZone))} คนมีค่าโซนไม่ตรงกฎโซนของ V1 จึงเทียบกับ Target รวม ${esc(fmt(S.zoneTargetOf({ key: 'unknown', group: '' })))} หยิบ/ชม. ต้องเติม Zone ให้ถูกก่อนจะเชื่อเงินของกลุ่มนี้ได้` : ''}<br>
        <b>ที่ทำไม่ได้เพราะ V3 ไม่มีข้อมูล</b> · หน้านี้ของ V2 มีโหมดคิดเป็น “ชิ้น” (pcs/UOM) และค่า Cycle Time ต่อหน่วย
        ซึ่งต้องใช้จำนวนชิ้นดิบ จำนวนบรรทัด และเวลาเริ่ม-จบระดับนาที — ข้อมูลผลงาน ไม่มีสามอย่างนี้ จึงคิดเป็น “หน่วยหยิบ” จากยอด Total Pick เท่านั้น
       · ไม่มี blendedTarget จึงใช้ Target ของโซน ตรง ๆ · ไม่มีโซนที่หยิบจริงรายรายการ (โซนในข้อมูลคือโซนสังกัด 1 แถวต่อคนต่อวัน)
